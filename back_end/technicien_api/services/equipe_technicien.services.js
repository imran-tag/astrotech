...
class EquipeTechnicienService {
  ...
  static async createEquipe({ nom, description, chefId }) {
    const [result] = await db.execute(
      'INSERT INTO equipe_technicien (nom, description, chefId) VALUES (?, ?, ?) RETURNING id',
      [nom, description, chefId]
    );
    return { id: result[0].id, nom, description, chefId };
  }
  ...
  static async addTechnicienToEquipe(equipeId, technicienId) {
    await db.execute(
      `INSERT INTO technicien_equipe (technicienId, equipeId)
       VALUES (?, ?)
       ON CONFLICT (technicienId, equipeId)
       DO UPDATE SET dateAffectation = CURRENT_TIMESTAMP`,
